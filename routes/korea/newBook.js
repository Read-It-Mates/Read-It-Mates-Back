...
async function crawlDetails($, element, index, intro) {
  ...
  let author = $(element)
    ...
    .text()
    .trim();
  let image = $(element).find(".front_cover").attr("src");
  if (image == null) {
    image = $(element).find(".i_cover").attr("src");
  }
...
router.post("/", async (req, res) => {
  try {
    // 베스트셀러 페이지 URL
    const bestURL =
      "https://www.aladin.co.kr/shop/common/wnew.aspx?ViewRowsCount=50&ViewType=Detail&SortOrder=6&page=1&BranchType=1&PublishDay=84&NewType=SpecialNew&SearchOption=#";
    ...
    const bestCollection = db.collection("newBook");
